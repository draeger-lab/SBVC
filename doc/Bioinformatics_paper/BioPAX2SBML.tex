...
\midrule
{\bf Signaling support (qual)}      & \checkmark                        & -                             & -  \\
...
{\bf Xrefs to CV terms}             & \checkmark                        & -                             & - \\
...
The checkmark (\checkmark) indicates that the criteria is completely fulfilled, the circles ($\circ$) means that the criteria is partly fulfilled, and the minus (-) that it is not fulfilled.
An conversion is \emph{valid} if the validators from SBML.org finds no errors in the converted SBML file and it is \emph{complete} if no BioPAX entity is missing in the converted model. 
...
A converter is \emph{robust} if it can handle bigger file sizes (more than 1MB) and is able to convert a BioPAX file which contains no \Pathway{} element.
...
Additionally, it is checked if the BioPAX cross-references (Xrefs) are converted into SBML controlled vocabulary terms (CV terms) and if the SBML model is \emph{augmented} with further database information. 
Finally, the \emph{provenance} indicates if the file history and the conversion is saved in the converted SBML file.
}
\end{table*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
