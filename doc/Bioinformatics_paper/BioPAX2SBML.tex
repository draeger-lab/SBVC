...
Finally, this converter is compared to existing conversion tools.
...
Just a few approaches exist to convert BioPAX to SBML and the existing ones use a simple one-to-one conversion.
...
Sybill \citep{Ruebenacker2009} and BiNoM \citep{Zinovyev2008} are two approaches that can be used for the translation of BioPAX into SBML but none of them is able to appropriately translating signaling networks.
...
Following, its main focus lies on the visualization of BioPAX files and not on the conversion.
We developed some comparison criteria and show the results in Table~\ref{tab:Comparison}.% shows the comparison of both approaches to the BioPAX2SBML converter approach.
...
Unfortunately, the converted SBML file is not complete and the validator from SBML.org reports errors, because species are used in several reactions that are missing in the species list. Additionally, some groups and pathway links are missing, too.
...
In contrast to Sybill, BiNoM converts BioPAX files without a pathway element, but it is just able to handle a small number of BioPAX files from the Pathway Interaction Database.
This is why both tools just partially fulfill the robustness criterion.
...
Finally, all approaches avoid the translation of dublicate species, but exclusively BioPAX2SBML converter uses the qual extension and SBO terms for detailed species description, translates the BioPAX cross-references (\texttt{Xrefs}) into SBML CV-terms, and augments the SMBL file content with further database cross-references.
...
The checkmark (\checkmark) indicates that the criteria is completely fulfilled, the circles ($\circ$) shows that the criterion is partially fulfilled, and the minus (-) is used if it is not fulfilled.
A conversion is \emph{valid} if the validator from SBML.org reports no errors in the converted SBML file and the converted model is \emph{complete} if no BioPAX entity is missing.
...
A converter is \emph{robust} if it can handle all tested files and is able to convert a BioPAX file, which contains no \Pathway{} element.
...
Additionally, it is checked if the BioPAX cross-references (\texttt{Xrefs}) are translated into SBML controlled vocabulary terms (CV terms) and if the SBML model is \emph{augmented} with further information.
