...
BioPAX2SBML is freely available at \href{http://webservices.cs.uni-tuebingen.de/}{http://webservices.cs.uni-tuebingen.de/} and the complete collection of the PID models at \href{http://www.cogsys.cs.uni-tuebingen.de/downloads/Qualitative-Models/}{http://www.cogsys.cs.uni-tuebingen.de/downloads/Qualitative-Models/}.
\section{Contact:}
...
All models are available at \href{http://www.cogsys.cs.uni-tuebingen.de/downloads/Qualitative-Models/}{http://www.cogsys.cs.uni-tuebingen.de/downloads/Qualitative-Models/}.
Furthermore, we provide our webtool BioPAX2SBML for further BioPAX translations at \href{http://webservices.cs.uni-tuebingen.de/}{http://webservices.cs.uni-tuebingen.de/}.
...
% There exist other conversion approaches but mostly 1:1 conversions
% It is difficult to convert BioPAX to SBML \cite{Bauer-Mehren2009}
% We augment and include qual
% What is important for the models (comparison table criteria...)
% For Sybill it is clear that qual could not be integrated, since exclusively Level 2 v4 is written
% auf die Besonderheit von BiNoM eingehen und erwähnen, dass der Vergleich eigentlich nicht ganz fair ist, weil es sich lediglich um ein Cytoscape plugin handelt (comparison to those tools is not reasonable)

Just a few approaches exist to convert BioPAX to SBML and most of them use a simple one-to-one conversion.
...
Sybill \citep{Ruebenacker2009} and BiNoM \citep{Zinovyev2008} are two approaches that can be used for the translation of BioPAX into SBML but none of them use the Qualitative Models extension.
Sybill is a stand-alone tool, that is also integrated in the quantitative modeling environment VCell \citep{Slepchenko2003}.
In contrast, BiNoM is a Cytoscape plugin which offers the possibility to open BioPAX Level~3 files but whose main focus lies on the visualization of BioPAX files.
We developed some comparison criteria and showed the results in Table~\ref{tab:Comparison}.% shows the comparison of both approaches to the BioPAX2SBML converter approach.
...
Unfortunately, the converted SBML file is not complete and the validator from SBML.org notifies errors, because species are used in several reactions that are missing in the species list. Additionally, some groups and pathway links are missing, too.
BiNoM produces complete files, but the validator also notifies errors due to empty \texttt{listOf} elements and due to the wrong order of these elements.
In contrast to Sybill, BiNoM converts BioPAX files without a pathway element, but it is not able to visualize BioPAX files bigger than 100kb.
That is why both tools just partly fulfill the robustness criterion.
...
Finally, all approaches prevent the translation of double species, but exclusively BioPAX2SBML converter uses the qual extension and SBO terms for detailed species describtion, translate the BioPAX cross-references (Xrefs) into SBML CV-terms, and augment the SMBL file content with further database links.
...
\begin{tabular}{lccc}
\toprule
{\bf }                              & {\bf BioPAX2SBML converter}       & {\bf Sybill}                  & {\bf BiNoM}\\
{\bf }                              & {B\"uchel \emph{et al.}}          & {Ruebenacker \emph{et al.}}   & {Zinovyev \emph{et al.}} \\
...
\midrule
{\bf Qualitative model}             & \checkmark                        & -                             & -  \\
{\bf support (qual)}                &                                   &                               &\\
...
{
The table compares three applications which are able to translate BioPAX into SBML.
The checkmark (\checkmark) indicates that the criteria is completely fulfilled, the circles ($\circ$) that the criteria is partly fulfilled, and the minus (-) that it is not fulfilled.
A conversion is \emph{valid} if the validator from SBML.org finds no errors in the converted SBML file and the converted model is \emph{complete} if no BioPAX entity is missing.
It is important for modeling purposes that there are no duplicate entities in the files, this is represented with the \emph{No duplicate entities} criterion.
A converter is \emph{robust} if it can handle bigger file sizes (more than 100KB) and is able to convert a BioPAX file which contains no \Pathway{} element.
The \emph{Compartments}, \emph{Stoichiometry}, and \emph{Uses SBO terms} criterion denotes if this information is translated into SBML and if the SBML species are denoted with the corresponding SBO term.
Additionally, it is checked if the BioPAX cross-references (Xrefs) are translated into SBML controlled vocabulary terms (CV terms) and if the SBML model is \emph{augmented} with further database information.
Finally, the \emph{provenance} criterion indicates if the file history and conversion tool information is saved in the converted SBML file.
}
\end{table*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%  \href{http://sbml.org/Community/Wiki/SBML\_Level\_3\_Proposals/Qualitative\_Models}{http://sbml.org/Community/Wiki/SBML\_Level\_3\_Proposals/Qualitative\_Models}.
...
%  \href{http://www.ebi.ac.uk/compneur-srv/sbml/converters/SBMLtoBioPAX.html}{http://www.ebi.ac.uk/compneur-srv/sbml/converters/SBMLtoBioPAX.html}.
%  Accessed 2012 Mar 22.
%
