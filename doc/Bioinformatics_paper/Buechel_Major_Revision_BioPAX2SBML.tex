...
\hyphenation{BioPAX BioCarta PaxTools SBML net-work net-works er-ro-neous-ly
      ...
      be-tween tran-scrip-tion unique-ly BioCyc}
...
In contrast, \NEW{SBML core exclusively describes quantitative processes such as reactions}.
With the SBML Qualitative Models extension (\qual), it has recently also become possible to describe relations in SBML.
Before the development of SBML \qual{}, relations could not be \NEW{properly translated into SBML}.
...
\section{Results:}
The entire Nature Pathway Interaction Database (PID) %, which includes pathways from BioCarta, Reactome, and the National Cancer Institute,
has been converted from BioPAX (Level~2 and Level~3) into SBML (Level~3 Version~1) including both reactions and relations by using the new \qual{} extension package.
Additionally, we present the new webtool BioPAX2SBML for further BioPAX to SBML conversions.
...
BioPAX can be used to describe the biological \NEW{semantics} of metabolic, signaling, molecular, gene-regulatory and genetic interaction networks \NEW{and is mainly used for qualitative analysis and information exchange} \citep{Demir2010}.
SBML describes the structure of models.
\NEW{In contrast to BioPAX, it} and offers the possibility to include mathematical expressions \NEW{which are necessary for dynamic simulations.} \citep{Hucka2003}.
\NEW{A detailed comparison between both languages can be found in \citet{Stromback2005, Stromback2006}.}

\NEW{Besides these language differences, BioPAX models from databases like the Nature Pathway Interaction Database (PID, \citealp{Schaefer2009}) or MetaCyc \citep{Caspi2012} are often used as information source to build SBML models for further simulation processes \citep{Bauer-Mehren2009}.
}
The SBML core specification defines \NEW{quantitative processes, such as reactions, events, rules, and constraints,} in detail but no other relationships between molecules.
...
\NEW{These relations are also provided in BioPAX models,} but before the creation of the Qualitative Models extension for SBML (\qual, \citealp[see][]{QualSpecification}), it was not possible to define those relations or to include reactions together with relations in one model.
%
...
Hence, new BioPAX to SBML converters are needed.
...
In this paper we present a webtool for the translation from BioPAX into SBML format.
We demonstrate its functionality by converting the whole PID from BioPAX Level~2 and Level~3 formats to the SBML format, including the \qual{} extension.
...
\NEW{The SBML core specification provides several constructs to describe quantitative processes, such as events, rules, constraints, reactions, etc., but there is no possibility to define qualitative relationships.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% FIGURE 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure*}[t!h]
\centering \includegraphics[width=0.9\textwidth]{Buechel_Figure1_BioPaxSBMLqual.tif}
...
\NEW{The used SBO terms are listed in Table~\ref{Tab:BioPAX2SBO}.}
...
Other applications, such as Clandestine \citep{Funahashi2007} or COPASI \citep{Hoops2006}, focus on visualization, simulation, analysis, etc. of those models.
...
In contrast, BiNoM is a Cytoscape plugin \citep{Smoot2011a}, which offers the possibility to open BioPAX Level~3 files.
...
\bibitem[Caspi {\em et~al.}(2012)Caspi, Altman, Dreher, Fulcher, Subhraveti,
Keseler, Kothari, Krummenacker, Latendresse, Mueller, Ong, Paley, Pujar,
Shearer, Travers, Weerasinghe, Zhang, and Kar]{Caspi2012}
Caspi, R., Altman, T., Dreher, K., Fulcher, C. A., Subhraveti, P.,
Keseler, I.~M., Kothari, A., Krummenacker, M., Latendresse, M.,
Mueller, L.~A., Ong, Q., Paley, S., Pujar, A.,
Shearer, A.~G., Travers, M., Weerasinghe, D., Zhang, P., and
Karp, P.~D. (2012).
\newblock The MetaCyc Database of Metabolic Pathways and Enzymes and the BioCyc Collection of Pathway/genome Databases.
\newblock {\em Nucleic Acids Res\/}, {\bf 40}(Database issue), D742–-D753.
...
\bibitem[Str\"omb\"ack {\em et~al.}(2006)Str\"omb\"ack, Jakoniene, Tan, Lambrix]{Stromback2006}
Str\"omb\"ack, L., Jakoniene, V., Tan, H. and Lambrix, P. (2006).
\newblock Representing, storing and accessing molecular interaction data: a review of models and tools.
\newblock {\em Briefings in Bioinformatics\/}, {\bf 7}(4), 331--338.

\bibitem[Str\"omb\"ack {\em et~al.}(2005)Str\"omb\"ack, Lambrix]{Stromback2005}
Str\"omb\"ack, L. and Lambrix, P. (2005).
\newblock Representations of molecular pathways: an evaluation of {SBML}, {PSI MI} and {BioPAX}.
\newblock {\em Bioinformatics\/}, {\bf 21}(24), 4401--4407.
