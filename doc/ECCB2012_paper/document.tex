...
The BioPax2SBML webservice is freely available at \url{http://webservices.cs.uni-tuebingen.de/} and the complete collection of the PID models at \url{http://www.cogsys.cs.uni-tuebingen.de/software/Qualitative-Models/}.
\section{Contact:}
...
The goal of systems biology is the model-driven understanding of biological and chemical processes across all layers and various levels of detail.
...
The BioPax specification is designed for the exchange, visualization, and analysis of such processes on a large scale.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% END FIGURE 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The complete Nature Pathway Interaction Database (PID) is converted from BioPax to SBML including the Qualitative Models extension (\qual).
PID provides curated pathways from the National Cancer Institute (L2/L3 2012-03-16), pathways from BioCarta (L2 2009-09-09, L3 2010-08-10), and human Reactome pathways (L2/L3 2010-08-10) \citep{Schaefer2009}.
...
% 3. Aber das Format ist ungeeignet
% 4. Daher gibt es mehrere Konverter, die folgendes können: reaction
The BioPax format is perfectly suitable to encode pathway relations and reactions that can be further used for visualizations or pathway analysis. However, this format also has its limitations.
...
Unfortunately, prior to the release of the SBML Qualitative Models extension (\qual), it was not possible to properly describe relations in SBML.
...
The \qual{} extension has only be recently released and thus, might not be supported by all applications yet.
...
Since older simulation applications still work with BioPax Level~2, we also translated these files into SBML to prevent loss of information and to be able to use these models, too.
...
Processes, such as the transport or modulation of an entity, can not directly be encoded as a reaction, at least not without knowing the exact chemical equation.
...
Furthermore, additional information is added beyond the scope of the BioPax document by utilizing the KEGG API.
