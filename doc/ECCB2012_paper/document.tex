...
\begin{abstract}
\section{Motivation:}
The Biological Pathway Exchange Language (BioPAX) and the Systems Biology Markup Language (SBML) belong to the two most popular modeling and data exchange languages in systems biology.
...
\section{Results:}
Here we present the complete Nature Pathway Interaction Database (PID), which includes pathways from BioCarta, Reactome, and from the National Cancer Institute in SBML qual format.
...
Furthermore, we provide a webservice for the translation from BioPAX into SBML.
{\TODO: {LEVEL 3 einfuegen}}
\section{Availability:}
The BioPAX2SBML webservice is freely available at \url{http://webservices.cs.uni-tuebingen.de/} and the complete collection of the PID models at \url{http://www.cogsys.cs.uni-tuebingen.de/downloads/Qualitative-Models/}.
\section{Contact:}
...
SBML describes the structure of models, and offers the possibility to include mathematical expressions.
...
Today, there exist mainly converters from SBML to BioPAX like \emph{The System Biology Format Converter} \citep[see][]{SBFC} but no converter for BioPAX to SBML that is capable of properly including (\TODO{include?})relations.
Other research groups previously faced the same problem with incompatibilities between BioPAX and SBML. {\TODO{Zitat?}}
To overcome the limitations of those file formats and to avoid the creation of pseudo-reactions or similar constructions, \citet{Ruebenacker2009} introduced an intermediate bridging format.
%
...
In this paper we present a webservice for the translation from BioPAX into SBML format. We showed its functionality by converting the whole Nature Pathway Interaction Database (PID, \citet{Schaefer2009} from BioPAX Level~2 and Level~3 formats to the SBML format, including the \qual{} extension.
...
The most important classes are \species{} that can be interconnected with \reaction{} elements.{\TODO{Repetition?}}
...
(\TODO{ shorten because of repetition?})
%%%%%%%%%%%%% replaced version
...
Instead of the quantities associated to \species, that are affected by reactions, \qualitativeSpecies{} exhibit discrete states, representing their activities, that are changed using transitions.
...
\emph{Dual} means that the transition can operate both as activating (\emph{positive}) and inhibiting (\emph{negative}).
In contrast, \emph{unknown} is assigned to the \inputP{} if the transition effect is not further specified.
If, in a qualitative model, the activity of protein A inhibits the activity of protein B, this would be represented as a transition with an input A which \sign{} attribute is \emph{negative} and an output B.
...
The BioPAX specification of Level~2 denotes all classes in lower case typewriter font and the specification of Level~3 denotes them in upper case typewriter font. For better readability of this paper, all BioPAX element names begin with capital letters and refer to Level~2 and 3.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% END FIGURE 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The complete Nature Pathway Interaction Database (PID) has been converted from BioPAX to SBML Core 3 including the Qualitative Models extension (\qual).
PID provides curated pathways from the National Cancer Institute (L2/L3 2012-03-16), pathways from BioCarta (L2 2009-09-09, L3 2010-08-10), and human Reactome pathways (L2/L3 2010-08-10 \citet{Schaefer2009}).
...
Firstly, the pathway organism is determined by searching for the BioSource reference in the BioPAX file.
...
Level~2 does not provide stoichiometric information. {\TODO: {That's wrong, rewrite}}

%%%%% Control
The translation of the \Conversion{} elements is straightforward, because all elements can unambiguously be mapped to reactions or relations.
The translation of \Control{} elements is more sophisticated, because translation into a transition or a reaction depends on enclosed \Control{} elements.
...
whose components can uniquely be identified by any application and be linked to external datasources.
\end{methods}
...
Many applications, especially for simulation and modeling of biological networks, use the SBML format.
Therefore, a few importers and converters for BioPAX2SBML have been developed.
Unfortunately, prior to the creation of the SBML Qualitative Models extension (\qual), it was not possible to properly describe relations in SBML.
...
Other applications focus on visualization, simulation, analysis, etc. of those models.
...
To match mRNA expression data or perform gene set enrichment analysis, Entrez Gene identifiers might be required.
...
The \qual{} extension has only be recently created and thus, might not be supported by all applications yet.
Therefore, we decided to build mixed SBML core and \qual{} models.
...
These models are compatible with older applications, that do not yet support \qual{} but still can read all \species{} and \reaction's.
But newer applications that are ready to handle relations can read the additional \qual{} model and process all information that was also available in the BioPAX file.
...
Furthermore, we provide a webservice for further BioPAX translations at \url{http://webservices.cs.uni-tuebingen.de/}.
...
Due to limitations of the SBML core specification, it was not possible to include all information from BioPAX files in SBML files, while producing
correct SBML code.
...
Using this extension, we produced error-free SBML and minimized or even eliminated the loss of information during the translation.
...
By utilizing the KEGG API it was even possible to complement the translated BioPAX documents with a wealth of information from the KEGG database.
...
We especially thank Johannes Eichner for help and support during the webservice establishment. 
%Additional thanks goes to the qualitative models and the JSBML team, which made this work possible.
Furthermore, we wish to acknowledge the qualitative models and the JSBML team.
